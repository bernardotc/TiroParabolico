/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package tiroparabolico;

import java.awt.Image;
import javax.swing.ImageIcon;
import java.awt.Toolkit;

/**
 *
 * @author mrquorr
 * @version 1.00 26/02/2014
 */
public class Caparazon extends Base {

    /**
     * Metodo constructor que hereda los atributos de la clase
     * <code>Base</code>.
     *
     * @param posX es la <code>posiscion en x</code> del objeto caparazon.
     * @param posY es el <code>posiscion en y</code> del objeto caparazon.
     *
     */
    public Caparazon(int posX, int posY) {
        super(posX, posY);
        this.velX = 0;
        this.velY = 0;
        Image imagen = Toolkit.getDefaultToolkit().getImage(this.getClass().getResource("images/redshell2.gif"));
        ImageIcon image = new ImageIcon(imagen);
        this.setImageIcon(image);
    }

    /**
     * Metodo que sirve para aplicar la gravedad al objeto <code>caparazon</code>.
     *
     * @param num es e; <code>gravedad</code>.
     *
     */
    public void gravedad(float num) {
        this.velY += num;
    }
}
